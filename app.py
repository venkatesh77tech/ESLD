...
On Render: this file reads the PORT env var Render assigns automatically —
do NOT set PORT yourself in the Render dashboard, just make sure the start
command is `python app.py` (or `gunicorn app:app` if you prefer).
"""
...
# Warn loudly at startup (in the Render logs) if DB env vars are missing,
# instead of failing silently on the first request a user makes.
_missing_db_vars = [v for v in ('MYSQL_HOST', 'MYSQL_USER', 'MYSQL_PASSWORD', 'MYSQL_DB') if not os.getenv(v)]
if _missing_db_vars:
    print(f"[ESLD][WARNING] Missing MySQL env vars: {_missing_db_vars}. "
          f"Set these in your Render service's Environment tab.")


# Catches anything that slips past individual route logic (e.g. MySQL
# connection refused because an env var is missing/wrong on Render, or any
# other unexpected exception) and returns clean JSON instead of an HTML 500
# error page — so the frontend's fetch() calls fail gracefully (and you can
# actually see what went wrong) instead of crashing on res.json().
...
#  ─────────────────────────────────────────
#  air-writing.html used to call api.anthropic.com DIRECTLY from the browser
#  with no API key — that request will always fail with 401 (browsers can't
#  send a secret key safely anyway), so the game was quietly running its
#  random-simulation fallback 100% of the time, never real recognition.
#  This route holds the key server-side instead, and the frontend now calls
#  this route via a *relative* URL (just "/recognize-letter") so it works
#  both on localhost and on the deployed Render origin.
#
#  Set your key once before running:
#     export ANTHROPIC_API_KEY=sk-ant-...      (macOS/Linux)
#     setx ANTHROPIC_API_KEY "sk-ant-..."       (Windows)
#  On Render: add ANTHROPIC_API_KEY in the service's Environment tab.
# ════════════════════════════════════════
...
if __name__ == '__main__':
    ...
    # env var. The old code hardcoded port=5000 — the app would "deploy"
    # successfully but never actually bind to the port Render expects, so
    # the live site would fail health checks / not load. Reading PORT here
    # fixes that; locally it just falls back to 5000 as before.
    port = int(os.environ.get('PORT', 5000))
    # debug=True in production leaks stack traces and auto-reloads on file
    # changes — fine on your laptop, not fine on a public Render URL.
    # Set FLASK_DEBUG=1 locally if you want debug mode back.
